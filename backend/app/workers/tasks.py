    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for clip_id, clip_path in clip_sources:
                clip_ext = Path(clip_path).suffix.lower()
                clip_artifact_key = f"jobs/{job_id}/inputs/{clip_id}{clip_ext}"
                with open(clip_path, "rb") as f:
                    payload = f.read()
                upload_bytes(clip_artifact_key, payload, _guess_video_mime(clip_ext))
                clip_manifest.append(
                    {"clip_id": clip_id, "key": clip_artifact_key, "size_bytes": len(payload)}
                )
            ...
            for clip_id, clip_path in clip_sources:
                ...
                if not cap.isOpened():
                    logger.warning("clip.unreadable", job_id=job_id, clip_id=clip_id, path=clip_path)
                    continue
                ...
                clip_duration = (total_frames / max(fps, 1)) if total_frames > 0 else 0.0
                ...
                while True:
                    ...
                    for points in per_track_points[clip_id].values():
                        if len(points) >= 2 and abs(points[-1]["t"] - t) < (
                            2 * sample_every / max(fps, 1)
                        ):
            ...
            for clip_id, tracks_map in per_track_points.items():
                for _, pts in tracks_map.items():
                    ...
                    track = Track(
                        ...
                        motion_stats_json={
                            ...
                            "avg_compensated_speed": float(
                                np.mean(
                                    [np.hypot(p.get("comp_dx", 0.0), p.get("comp_dy", 0.0)) for p in pts]
                                )
                            ),
                        },
                    )
                    ...
                    if class_name in VEHICLE_CLASSES:
                        ...
                        close_conf = _safe_conf(
                            close_following_confidence(pts, output_size[0] if output_size else 1280)
                        )
                        if close_conf > 0.2:
                            db.add(
                                Event(
                    ...
                    if class_name == "bicycle":
                        bike_conf = _safe_conf(
                            bike_proximity_confidence(pts, output_size[0] if output_size else 1280)
                        )
                        if bike_conf > 0.2:
                            db.add(
                                Event(
            ...
            # Export tracks NDJSON
            with open(tracks_path, "w", encoding="utf-8") as f:
                for track in tracks:
                    points = track_rows_points.get(track.id) or []
                    step = max(1, len(points) // 20 or 1)
                    sampled = [{"t": p["t"], "x": p["xc"], "y": p["yc"]} for p in points[::step]]
                    row = {
                        "datapack_version": DATAPACK_VERSION,
            ...
            job.logs_summary = (
                ...
                f"tracks={sum(class_counts.values())}, events={sum(event_counts.values())}, "
                f"artifacts={len(artifact_manifest)}"
            )

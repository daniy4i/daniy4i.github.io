...
def compute_window_metrics(active_tracks: int, avg_motion: float) -> dict[str, float]:
    density_index = min(1.0, active_tracks / 20.0)
    speed_norm = min(1.0, max(0.0, avg_motion / 25.0))
    stopped_ratio = max(0.0, min(1.0, 1.0 - speed_norm))
    avg_speed_proxy = round(avg_motion, 3)
    return {
